...
from src.utils import from_file_to_boundingbox
...
for batch in dataloader_valid:
    ...
    gt_boxes = list(
        chain(*[from_file_to_boundingbox(target, name=name, groundtruth=True) for target, name in zip(y, x_name)]))
    pred_boxes = list(
        chain(*[from_file_to_boundingbox(pred, name=name, groundtruth=False) for pred, name in zip(preds, x_name)]))
